...
Pigment-shaped examples (`css` / `styled` from `@pigment-css/react`, MUI `sx`, theme tokens, `:focus-visible` in CSS) are the ease-of-use default. Behavior is required; class-name spelling is not. Map the same rules onto the framework already in the app.
...
- Interactive elements need visible focus: `:focus-visible` outline or box-shadow (`css` / `sx` / theme), or equivalent
...
- Text containers handle long content: overflow truncation, `line-clamp`, or overflow-wrap (`css` / `sx`)
- Flex children need `minWidth: 0` / `min-width: 0` to allow text truncation
...
- Buttons/links need a `:hover` (or `sx` `&:hover`) visual feedback state
