...
> Vendored from Anthropic skills (`frontend-design`), Apache-2.0.
> Unmodified snapshot: `third_party/anthropics-skills/frontend-design/`.
> Snapshot revision: 34040c9c568585f6929bedeaad110ad08f079624.
> This file is a **Cursor port** (not unmodified).

## Copy and interaction (this Cursor port)

Implementation follows `web-design-guidelines` for copy, labels, and interaction quality (Vercel). Aesthetic direction, type, palette, layout, motion restraint, and anti-generic-kit visual tells stay Anthropic **except** where they contradict Vercel copy.

Do **not** use Anthropic sentence-case for headings or buttons. Title Case on headings and buttons (Chicago), active voice, numerals for counts, specific button labels, second person, `…` not `...`, curly quotes.

## Styling stack (this Cursor port)
...
If the **app repo already has a styling framework in use, keep it. Do not add a parallel stack.** If bootstrapping a new UI (or the app has only unopinionated global/vanilla CSS and no component or utility framework), prefer **MUI (`@mui/material`) + Pigment CSS (`@pigment-css/react`, MUI’s `@mui/material-pigment-css` integration)**.

- Framework already present → use that one. Examples: MUI (including Emotion-as-the-system), Pigment CSS, Tailwind, Chakra, Ant Design, Mantine, Bootstrap, styled-components, vanilla-extract, Panda, UnoCSS, shadcn/Radix+Tailwind already wired.
- Only Pigment CSS present → keep Pigment; **do not** add MUI beside it.
- Only MUI present → keep MUI; **do not** add Pigment beside it.
- Both already present → keep both.
- CSS Modules / a hand-written `globals.css` with no framework **is not** a styling framework. Bootstrap default is MUI + Pigment CSS.

**Next + Pigment: drop Turbopack.** Pigment’s Next plugin (`withPigment`) is webpack-only. New apps that take the MUI+Pigment default must **not** use `next dev --turbopack` / `next build --turbopack`. Use webpack (`next dev` / `next build` without `--turbopack`). If the repo already uses Turbopack **and** is not adopting Pigment, keep Turbopack. Set `experimental.optimizePackageImports` (or current Next equivalent) for `@mui/material` so this stack does not fight `bundle-barrel-imports`.

Do **not** mandate Tailwind or shadcn as the default.

Use this skill for new UI, restyle, “looks generic/basic,” layout/typography/color. Not API-only `.ts`.
...
Keep the tone conversational: plain verbs, no filler, with tone matched to the brand and the audience. Let each written element do exactly one job. For headings, buttons, labels, and interaction quality, follow `web-design-guidelines` (Title Case, not sentence case).
